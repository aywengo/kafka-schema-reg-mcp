    ...
    try:
        ...
        if needs_elicitation:
            ...
            if response and response.complete:
                ...
                logger.info(f"Built schema definition from elicitation: {len(schema_definition.get('fields', []))} fields")
            else:
...
async def migrate_context_interactive(
    ...
    try:
        ...
        if needs_elicitation:
            logger.info(f"Context migration from {source_registry} to {target_registry} needs preferences: {missing_preferences}")
            ...
            if response and response.complete:
                ...
                logger.info(f"Applied migration preferences from elicitation: preserve_ids={preserve_ids}, dry_run={dry_run}, migrate_all_versions={migrate_all_versions}")
            else:
                return create_error_response(
        ...
        if isinstance(compatibility_result, dict):
            ...
            if not is_compatible and compatibility_errors:
                ...
                if response and response.complete:
                    ...
                    logger.info(f"Added compatibility resolution guidance: {response.values.get('resolution_strategy')}")
                else:
    ...
    try:
        ...
        if isinstance(result, dict) and "error" not in result:
            ...
            if elicited_metadata:
                ...
                result["metadata_note"] = "Context created with elicited metadata. Store this information in your documentation system."
    ...
    try:
        ...
        if needs_elicitation:
            ...
            if response and response.complete:
                ...
                if include_config is None:
                    include_config = response.values.get("include_metadata", "true").lower() == "true"  # Use same setting
                ...
                logger.info(f"Applied export preferences from elicitation: format={format}, versions={include_versions}")
            else:
                # Use safe defaults
        ...
        if isinstance(result, dict) and "error" not in result:
            ...
            if compression and compression != "none":
                result["compression_note"] = f"Consider compressing the export with {compression} for storage efficiency"
...
async def _build_schema_from_elicitation(
    ...
    if schema_type == "AVRO":
        ...
        return {
            "type": schema_type,
            "elicited_fields": elicited_values
        }
