...
async def test_readonly_mode():
    ...
    print(f"READONLY setting: {kafka_schema_registry_mcp.READONLY}")
    ...
    readonly_check = kafka_schema_registry_mcp.check_readonly_mode()
    if readonly_check:
    ...
    blocked_functions = [
        ("register_schema", lambda: kafka_schema_registry_mcp.register_schema(
            "test-subject", {"type": "string"})),
        ("create_context", lambda: kafka_schema_registry_mcp.create_context("test-context")),
        ("delete_context", lambda: kafka_schema_registry_mcp.delete_context("test-context")),
        ("delete_subject", lambda: kafka_schema_registry_mcp.delete_subject("test-subject")),
        ("update_global_config", lambda: kafka_schema_registry_mcp.update_global_config("BACKWARD")),
        ("update_subject_config", lambda: kafka_schema_registry_mcp.update_subject_config(
            "test-subject", "BACKWARD")),
        ("update_mode", lambda: kafka_schema_registry_mcp.update_mode("READONLY")),
        ("update_subject_mode", lambda: kafka_schema_registry_mcp.update_subject_mode(
    ...
    allowed_functions = [
        ("list_contexts", lambda: kafka_schema_registry_mcp.list_contexts()),
        ("list_subjects", lambda: kafka_schema_registry_mcp.list_subjects()),
        ("get_global_config", lambda: kafka_schema_registry_mcp.get_global_config()),
        ("get_mode", lambda: kafka_schema_registry_mcp.get_mode()),
    ]
    ...
    export_functions = [
        ("export_schema", lambda: kafka_schema_registry_mcp.export_schema("test-subject")),
        ("export_subject", lambda: kafka_schema_registry_mcp.export_subject("test-subject")),
        ("export_context", lambda: kafka_schema_registry_mcp.export_context("test-context")),
        ("export_global", lambda: kafka_schema_registry_mcp.export_global()),
    ]
    ...
    try:
        result = kafka_schema_registry_mcp.check_compatibility(
            "test-subject", {"type": "string"})
        if isinstance(result, dict) and "readonly_mode" in result:
...
async def test_readonly_environment_variations():
    ...
    for value, expected in test_values:
        ...
        importlib.reload(kafka_schema_registry_mcp)
        actual = kafka_schema_registry_mcp.READONLY
