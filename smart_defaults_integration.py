...
class SmartDefaultsElicitationEnhancer:
    ...
    async def enhance_elicitation_request(
        ...
        if operation == "create_schema" and "_field_suggestions" in defaults:
    ...
    def _enhance_field_with_defaults(
        ...
        if field.name in defaults:
            ...
            if enhanced.description:
                enhanced.description += f" (Suggested: {smart_default.value} - {int(smart_default.confidence * 100)}% confidence)"
            else:
                enhanced.description = f"Suggested: {smart_default.value} - {int(smart_default.confidence * 100)}% confidence"
    ...
    def _enhance_description(self, description: Optional[str], defaults: Dict[str, SmartDefault]) -> Optional[str]:
        ...
        high_confidence_count = len([
            d for d in defaults.values() 
            if isinstance(d, SmartDefault) and d.confidence >= 0.7
        ])
    ...
    def _add_field_suggestion_metadata(
        self, 
        request: ElicitationRequest, 
        field_suggestions: Dict[str, List[FieldSuggestion]]
    ):
...
async def create_enhanced_export_elicitation(
    ...
    enhanced_request = await enhancer.enhance_elicitation_request(
        request=request,
        operation=f"export_{operation_type}",
        context=context,
        existing_data=existing_data
    )
...
def enable_smart_defaults_globally(registry_manager: Optional[SchemaRegistryManager] = None):
    ...
    async def enhanced_schema_field(*args, **kwargs):
        ...
        return await enhancer.enhance_elicitation_request(
            base_request, "create_schema_field", kwargs.get("context")
        )
    ...
    async def enhanced_migration(*args, **kwargs):
        ...
        return await enhancer.enhance_elicitation_request(
            base_request, "migrate_schema", kwargs.get("context")
        )
    ...
    async def enhanced_context(*args, **kwargs):
        ...
        return await enhancer.enhance_elicitation_request(
            base_request, "create_context", context_name
        )
    ...
    async def enhanced_export(*args, **kwargs):
        ...
        return await enhancer.enhance_elicitation_request(
            base_request, f"export_{operation_type}", None
        )
