    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            multi_registry_tools = [
                ...
                "migrate_schema",
                "migrate_context",  # Now generates Docker configuration
                "list_migrations",
            ...
            try:
                ...
                if result.content and len(result.content) > 0:
                    ...
                    for registry in registries:
                        print(
                            f"   • {registry.get('name', 'unknown')}: {registry.get('url', 'no url')}"
                        )
                else:
            ...
            try:
                ...
                if result.content and len(result.content) > 0:
                    ...
                    if "error" in comparison:
                        ...
                        print(
                            f"✅ Comparison completed: {comparison.get('source')} vs {comparison.get('target')}"
                        )
