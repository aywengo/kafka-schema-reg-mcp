...
async def test_security_and_compliance():
    ...
    try:
        # Add timeout to prevent hanging
        await asyncio.wait_for(
            _test_security_compliance_with_client(server_params),
            timeout=60.0
        )
    except asyncio.TimeoutError:
...
async def test_monitoring_and_observability():
    ...
    try:
        # Add timeout to prevent hanging
        await asyncio.wait_for(
            _test_monitoring_observability_with_client(server_params),
            timeout=60.0
        )
    except asyncio.TimeoutError:
...
async def test_disaster_recovery():
    ...
    try:
        # Add timeout to prevent hanging
        await asyncio.wait_for(
            _test_disaster_recovery_with_client(server_params),
            timeout=60.0
        )
    except asyncio.TimeoutError:
