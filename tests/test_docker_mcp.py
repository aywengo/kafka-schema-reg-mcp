...
async def test_docker_mcp_server():
    ...
    try:
        ...
        docker_cmd = [
            ...
            "--name", container_name,
            "--network", "host",
            "-e", "SCHEMA_REGISTRY_URL=http://localhost:38081",
            "kafka-schema-reg-mcp:test",
            "python", "-c", "import kafka_schema_registry_mcp; print('MCP server test mode'); import time; time.sleep(60)"
        ]
        ...
        health_cmd = ["docker", "exec", container_name, "python", "-c", "print('Container is responsive')"]
