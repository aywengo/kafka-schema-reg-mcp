    ...
    try:
            ...
            try:
                ...
                if version_response.status_code != 200:
                    migration_results["failed"].append({
                        "version": version,
                        "reason": "Failed to retrieve from DEV"
                    })
                ...
                if migrate_response.status_code == 200:
                    ...
                    migration_results["failed"].append({
                        "version": version,
                        "reason": "PROD registry is read-only"
                    })
                    ...
                    print(f"   ❌ Version {version} migration failed: {migrate_response.status_code}")
            ...
            for i in range(len(all_versions) - 1):
                ...
                if current_resp.status_code == 200:
                    ...
                    if compat_resp.status_code == 200:
                        ...
                        print(f"   Version {next_version} → {current_version}: {'✅ Compatible' if is_compatible else '❌ Incompatible'}")
                    else:
                        print(f"   Version {next_version} → {current_version}: ⚠️  Compatibility check failed")
        ...
        if created_versions:
            ...
            for version_info in created_versions:
                print(f"   • {version_info['name']}: v{version_info.get('version', 'unknown')} (ID: {version_info.get('id', 'unknown')})")
