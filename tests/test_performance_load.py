...
async def timed_operation(
    session: ClientSession, operation: str, tool_name: str, params: Dict[str, Any]
) -> bool:
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print(
                    f"  ✅ Sequential registration: {seq_stats['avg_time_ms']:.2f}ms avg, {seq_stats['success_rate']:.2%} success"
                )
                ...
                print(
                    f"  ✅ List subjects: {list_stats['avg_time_ms']:.2f}ms avg, {list_stats['success_rate']:.2%} success"
                )
                ...
                print(
                    f"  ✅ Get schema: {get_stats['avg_time_ms']:.2f}ms avg, {get_stats['success_rate']:.2%} success"
                )
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                for i in range(5):
                    await timed_operation(
                        session, "test_all_registries_perf", "test_all_registries", {}
                    )
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                for field_count in field_counts:
                    large_schema = generate_test_schema(
                        f"LargeSchema{field_count}Fields", field_count
                    )
                    await timed_operation(
                        session,
                        f"register_large_{field_count}",
                        "register_schema",
                        {
            ...
            async with ClientSession(read, write) as session:
                ...
                for op_type in [
                    ...
                    if op_type in metrics.operation_times:
                        stats = metrics.get_stats(op_type)
                        print(
                            f"     {op_type}: {stats['avg_time_ms']:.2f}ms avg, {stats['success_rate']:.2%} success"
                        )
                ...
                print(
                    f"  ✅ Memory stress test: {memory_stats['avg_time_ms']:.2f}ms avg, {memory_stats['success_rate']:.2%} success"
                )
