...
class OAuthDiscoveryTest:
            ...
            for i in range(30):  # Wait up to 30 seconds
                ...
                for test_port in [8899, 8000]:  # Try both ports
                    try:
                        ...
                        if response.status_code in [200, 503]:  # 503 is OK if registries not available
    ...
    def test_oauth_protected_resource_endpoint(self) -> bool:
        ...
        try:
            ...
            mcp_fields = ["mcp_server_info", "scope_descriptions", "protected_endpoints"]
            for field in mcp_fields:
                if field in data:
                    print(f"   ✅ Found MCP extension: {field}")
                else:
    ...
    def test_discovery_consistency(self) -> bool:
        ...
        try:
            ...
            protected_resource_resp = requests.get(f"{self.server_url}/.well-known/oauth-protected-resource", timeout=10)
            
            # Both should have same success status
            if auth_server_resp.status_code != protected_resource_resp.status_code:
                print(f"   ⚠️  Status code mismatch: auth_server={auth_server_resp.status_code}, protected_resource={protected_resource_resp.status_code}")
    ...
    def test_pkce_mandatory_requirements(self) -> bool:
        ...
        try:
            ...
            if set(pkce_methods) != set(resource_pkce_methods):
                print(f"   ⚠️  PKCE method mismatch between endpoints: auth={pkce_methods}, resource={resource_pkce_methods}")
            else:
            ...
            pkce_compliant = (
                protected_data.get("require_pkce") is True and
                "S256" in resource_pkce_methods and
                "mandatory" in protected_data.get("pkce_note", "").lower()
            )
    ...
    def run_all_tests(self) -> bool:
        ...
        try:
            # Run all tests
            tests = [
                ("OAuth Authorization Server Endpoint", self.test_oauth_authorization_server_endpoint),
                ...
                ("Different OAuth Providers", self.test_discovery_with_different_providers),
        ...
        try:
            ...
            self.run_test("OAuth Disabled - Authorization Server 404", self.test_oauth_authorization_server_endpoint)
            self.run_test("OAuth Disabled - Protected Resource 404", self.test_oauth_protected_resource_endpoint)
