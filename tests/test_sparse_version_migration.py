...
class SparseVersionMigrationTest:
    ...
    async def create_sparse_version_schema(self, subject: str):
        ...
        try:
            ...
            url1 = dev_client.build_context_url(f"/subjects/{subject}/versions/1", self.source_context)
            response1 = requests.delete(url1, auth=dev_client.auth, headers=dev_client.headers)
            if response1.status_code == 200:
                print("✓ Deleted version 1")
            else:
                print(f"Warning: Could not delete version 1: {response1.status_code} - {response1.text}")
        except Exception as e:
            print(f"Warning: Error deleting version 1: {e}")
        
        # Delete version 2  
        try:
            url2 = dev_client.build_context_url(f"/subjects/{subject}/versions/2", self.source_context)
            response2 = requests.delete(url2, auth=dev_client.auth, headers=dev_client.headers)
            if response2.status_code == 200:
                print("✓ Deleted version 2")
            else:
                print(f"Warning: Could not delete version 2: {response2.status_code} - {response2.text}")
        except Exception as e:
            print(f"Warning: Error deleting version 2: {e}")
        
        # Verify we now have sparse versions [3, 4, 5]
        final_versions = mcp_server.get_schema_versions(subject, context=self.source_context, registry="dev")
        ...
        if sorted(final_versions) != expected_versions:
            raise Exception(f"Expected sparse versions {expected_versions}, got {sorted(final_versions)}")
    ...
    async def verify_sparse_versions_preserved(self, subject: str, expected_versions: list):
        ...
        target_versions = mcp_server.get_schema_versions(subject, context=self.target_context, registry="prod")
