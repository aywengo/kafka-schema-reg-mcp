...
class TestElicitationIntegration:
    ...
    async def test_end_to_end_schema_registration_workflow(self):
        ...
        mock_register_tool = Mock(return_value={
            "success": True,
            "id": 123,
            "subject": subject,
            "version": 1
    ...
    async def test_migration_preferences_elicitation_workflow(self):
        ...
        mock_migrate_tool = AsyncMock(return_value={
            "success": True,
            "migrated_schemas": 5,
            "dry_run": True,
            "preserved_ids": True
    ...
    async def test_context_metadata_collection(self):
        ...
        mock_create_tool = Mock(return_value={
            "success": True,
        ...
        assert result["metadata"]["tags"] == ["analytics", "events", "pipeline", "gdpr-compliant"]
    ...
    async def test_export_preferences_workflow(self):
        ...
        # Mock export tool
        mock_export_tool = Mock(return_value={
            "success": True,
            "exported_schemas": 25,
            "export_format": "json",
            "file_size": "2.4MB"
    ...
    async def test_multi_round_elicitation_workflow(self):
        ...
        with patch('interactive_tools.elicit_with_fallback', side_effect=mock_elicit_multiple_rounds):
            result = await register_schema_interactive(
                subject=subject,
                schema_definition={"type": "record", "name": "UserProfile", "fields": []},
                register_schema_tool=mock_register_tool,
                registry_manager=self.mock_registry_manager,
                registry_mode=self.registry_mode,
                auth=self.mock_auth,
    ...
    async def test_enhanced_elicitation_with_fallback_chain(self):
        ...
        from elicitation_mcp_integration import enhanced_elicit_with_fallback, set_mcp_instance
        
        # Test 1: Real MCP elicitation succeeds
        mock_mcp = Mock()
        mock_mcp.send_elicitation_request = AsyncMock(return_value={
            "values": {"field": "real_mcp_value"}, 
            "complete": True
        })
