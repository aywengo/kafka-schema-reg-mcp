        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                result = await session.call_tool("test_registry_connection", {
                    "registry_name": "invalid_registry"
                })
                ...
                result = await session.call_tool("get_registry_info", {
                    "registry_name": "invalid_registry"
                })
                response = json.loads(result.content[0].text) if result.content else {}
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                edge_case_subjects = ["", " ", "subject with spaces", "subject-with-special-chars!@#"]
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("\nTest 5: Sync operations with mixed registry states")
                result = await session.call_tool("sync_schema", {
                    "subject": "test-subject",
                    "source_registry": "valid_registry",
                    "target_registry": "readonly_registry",
                    "dry_run": True
