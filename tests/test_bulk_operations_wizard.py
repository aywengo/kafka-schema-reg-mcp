...
from bulk_operations_wizard import (
...
class TestBulkOperationsWizard:
    ...
    def mock_registry(self):
        ...
        registry = Mock()
        registry.list_subjects = AsyncMock(return_value=[
            "test-schema-1",
            "test-schema-2",
            "prod-schema-1",
            "deprecated-schema-1",
    ...
    def mock_task_manager(self):
        ...
        task_manager.get_task_status = AsyncMock(return_value={
            "status": "completed",
            "processed": 5,
            "total_items": 5
        })
    ...
    async def test_schema_selection_simple(self, wizard):
        ...
        # Mock elicitation response
        wizard.elicitation.elicit.return_value = {
            "values": ["test-schema-1", "test-schema-2"]
        }
    ...
    async def test_schema_selection_with_patterns(self, wizard):
        ...
        wizard.elicitation.elicit.return_value = {
            "values": ["test-*", "prod-schema-1"]
        }
        
        result = await wizard._elicit_schema_selection(
            "Select schemas",
            allow_patterns=True
        )
    ...
    async def test_preview_generation(self, wizard):
        ...
        preview = await wizard._generate_preview(
            BulkOperationType.SCHEMA_UPDATE,
            items,
            params
        )
    ...
    async def test_execute_operation_success(self, wizard):
        ...
        wizard._process_batch = AsyncMock(return_value={
            "batch_size": 2,
            "success": 2,
            "failed": 0,
            "errors": []
        ...
        result = await wizard._execute_bulk_operation(
            BulkOperationType.SCHEMA_UPDATE,
            items,
            params,
            preview
    ...
    async def test_bulk_cleanup_workflow(self, wizard):
        ...
        # Mock other methods
        wizard._generate_preview = AsyncMock(return_value=BulkOperationPreview(
        ...
        wizard._execute_bulk_operation = AsyncMock(return_value={
            "status": "success",
            "processed": 1
        })
