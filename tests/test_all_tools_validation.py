#!/usr/bin/env python3
"""
Comprehensive Tool Validation for unified server in multi-registry mode
...
# Expected resources
EXPECTED_RESOURCES = ["registry://status", "registry://info"]
...
class ToolValidator:
    ...
    async def validate_tool(
        self, tool_name: str, test_params: Dict[str, Any] = None
...
async def test_all_tools_comprehensive():
    ...
    try:
        ...
        await asyncio.wait_for(
            _test_all_tools_with_client(server_params, validator),
            timeout=120.0,  # 2 minute timeout for comprehensive testing
        )
    except asyncio.TimeoutError:
        print("❌ All tools validation test timed out after 120 seconds")
    except Exception as e:
        print(f"❌ All tools validation test failed: {e}")


async def _test_all_tools_with_client(server_params, validator):
    """Helper function for all tools test with timeout protection."""
        ...
        async with ClientSession(read, write) as session:
            ...
            for tool_name in EXPECTED_TOOLS["registry_management"]:
                if tool_name == "list_registries":
                    ...
                    result = await validator.validate_tool(
                        tool_name, {"registry_name": "validation_primary"}
                    )
            ...
            for tool_name in EXPECTED_TOOLS["migration"]:
                if tool_name == "migrate_schema":
                    ...
                    result = await validator.validate_tool(
                        tool_name, {"migration_id": "test-migration-id"}
                    )
            ...
            for tool_name in EXPECTED_TOOLS["context_management"]:
                if tool_name == "list_contexts":
                    result = await validator.validate_tool(
                        tool_name, {"registry": "validation_primary"}
                    )
            ...
            print(
                f"• Fastest tool: {fastest_tool.tool_name} ({fastest_tool.response_time_ms:.2f}ms)"
            )
            print(
                f"• Slowest tool: {slowest_tool.tool_name} ({slowest_tool.response_time_ms:.2f}ms)"
            )
...
async def main():
    ...
    print("Testing all 68+ MCP tools for unified server in multi-registry mode")
