...
from elicitation import ElicitationRequest, ElicitationField, ElicitationType
...
class TestLearningEngine:
    ...
    def test_record_and_retrieve_choice(self):
        ...
        # Retrieve preference
        preference = self.learning_engine.get_historical_preference(
            "create_schema", "production", "compatibility"
        )
    ...
    def test_persistence(self):
        ...
        preference = new_engine.get_historical_preference(
            "create_context", "test", "environment"
        )
...
class TestSmartDefaultsEngine:
    ...
    async def test_context_specific_defaults(self):
        ...
        # Development context
        dev_defaults = await engine.suggest_defaults(
            operation="create_schema",
            context="development"
        )
        ...
        # Production context
        prod_defaults = await engine.suggest_defaults(
            operation="create_schema",
            context="production"
        )
    ...
    async def test_existing_data_override(self):
        ...
        defaults = await engine.suggest_defaults(
            operation="create_schema",
            context="production",
            existing_data={"compatibility": "BACKWARD"}
        )
    ...
    async def test_feedback_processing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            request = ElicitationRequest(
                ...
                title="Test",
                fields=[
                    ElicitationField(name="test_field", type="text")
                ]
            )
            ...
            response = ElicitationResponse(
                request_id=enhanced.id,
                values={"test_field": "user_value"}
            )
...
class TestEnhancedElicitationFunctions:
    ...
    async def test_enhanced_migration_elicitation(self):
        ...
        preserve_ids_field = next(
            (f for f in request.fields if f.name == "preserve_ids"), None
        )
