...
class TestBatchCleanupIntegration:
        ...
        for context in cls.test_contexts:
            try:
                ...
                for registry_url in [cls.dev_url, cls.prod_url]:
                    ...
                    subjects_response = requests.get(f"{registry_url}/contexts/{context}/subjects", timeout=5)
    ...
    def test_large_context_performance(self):
        ...
        subjects = self._create_test_context_with_subjects(context_name, self.dev_url, subject_count)
    ...
    async def test_cross_registry_operations(self):
        ...
        dev_result = multi_mcp.clear_context_batch(context=context_name, registry="dev", dry_run=True)
        ...
        prod_result = multi_mcp.clear_context_batch(context=context_name, registry="prod", dry_run=True)
        ...
        dev_cleanup = multi_mcp.clear_context_batch(context=context_name, registry="dev", dry_run=False)

        prod_cleanup = multi_mcp.clear_context_batch(context=context_name, registry="prod", dry_run=False)
    ...
    def test_context_deletion_after_cleanup(self):
        ...
        result = single_mcp.clear_context_batch(context=context_name, delete_context_after=True, dry_run=False)

        assert result["context_deleted"] or "context" in str(result), "Should attempt context deletion"
    ...
    def test_error_recovery_and_reporting(self):
        ...
        valid_result = multi_mcp.clear_context_batch(context=context_name, registry="dev", dry_run=False)
