...
def initialize_smart_defaults(registry_manager: Optional[SchemaRegistryManager] = None) -> bool:
    ...
    try:
        ...
        logger.info(f"  - Confidence Thresholds: suggestion={config.min_confidence_for_suggestion}, auto-fill={config.min_confidence_for_auto_fill}")
        logger.info(f"  - Privacy: anonymize={'yes' if config.anonymize_values else 'no'}, excluded_fields={len(config.excluded_fields)}")
