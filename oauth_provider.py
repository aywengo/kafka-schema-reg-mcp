    ...
    try:
        ...
        class KafkaSchemaRegistryOAuthProvider(OAuthAuthorizationServerProvider):
            ...
            def extract_scopes_from_jwt(self, jwt_payload: Dict[str, Any]) -> Dict[str, Any]:
                ...
                logger.info(f"Extracted scopes for user {jwt_payload.get('sub', 'unknown')}: {user_scopes}")
            ...
            async def validate_jwt_token(self, token: str) -> Optional[Dict[str, Any]]:
                ...
                try:
                    ...
                    public_key = await self.get_public_key_from_jwks(config["jwks_url"], unverified_header.get("kid"))
                    if not public_key:
                    ...
                    logger.info(f"JWT token validated successfully for user: {validated_payload.get('sub', 'unknown')}")
                ...
                try:
                    ...
                    for key_data in jwks_data.get("keys", []):
                        if key_data.get("kid") == kid and key_data.get("kty") == "RSA":
                            ...
                            logger.debug(f"Successfully fetched and cached public key for kid: {kid}")
                            return pem_key
    ...
    return {
        "azure": {
            ...
            "jwt_validation": {
                ...
                "algorithms": ["RS256"],
                "claims": {"iss": "Issuer validation", "aud": "Audience validation", "exp": "Expiration check"}
            },
        ...
        "google": {
            ...
            "jwt_validation": {
                ...
                "algorithms": ["RS256"],
                "claims": {"iss": "https://accounts.google.com", "aud": "Client ID", "exp": "Expiration check"}
            },
        ...
        "keycloak": {
            ...
            "jwt_validation": {
                ...
                "algorithms": ["RS256"],
                "claims": {"iss": "Realm issuer URL", "aud": "Client ID", "exp": "Expiration check"}
            },
