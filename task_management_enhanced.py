...
class EnhancedAsyncTaskManager:
    ...
    def create_task(
        ...
        if context:
            asyncio.create_task(self._log_task_event(
                context,
                f"Created {task_type.value} task",
                task_id=task_id,
                level="info"
            ))
        ...
        try:
            ...
            if context:
                await self._log_task_event(
                    context,
                    f"Started executing {task.type.value} task",
                    task.id,
                    level="info"
                )
            ...
            try:
                ...
                if context:
                    await self._log_task_event(
                        context,
                        f"Completed {task.type.value} task successfully",
                        task.id,
                        level="info"
                    )
                ...
                if context:
                    await self._log_task_event(
                        context,
                        f"Cancelled {task.type.value} task",
                        task.id,
                        level="warning"
                    )
                ...
                if context:
                    await self._log_task_event(
                        context,
                        f"Failed {task.type.value} task: {str(e)}",
                        task.id,
                        level="error"
                    )
    ...
    async def update_progress(
        ...
        if task._context and task._progress_token:
            ...
            if abs(task.progress - task._last_reported_progress) > 1.0:
                try:
                    await task._context.report_progress(
                        task.progress,
                        100.0,
                        message
                    )
...
def create_context_aware_task(
    ...
    if not hasattr(create_context_aware_task, '_manager'):
        create_context_aware_task._manager = EnhancedAsyncTaskManager()
        
    return create_context_aware_task._manager.create_task(
        task_type,
        metadata,
        context
    )
...
class ProgressReporter:
    ...
    class Phase:
        ...
        async def update_item(self, index: int):
            ...
            await self.reporter.update(
                progress,
                f"{self.message} - {self.current}/{self.total}"
            )
