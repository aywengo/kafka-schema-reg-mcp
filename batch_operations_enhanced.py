...
from task_management_enhanced import (
    TaskType,
    create_context_aware_task,
    enhanced_task_manager,
    ProgressReporter
)
...
async def clear_context_batch_enhanced_tool(
    ...
    try:
        ...
        if ctx:
            await ctx.error(
                f"Failed to start batch operation: {str(e)}",
                logger_name="BatchOperations"
            )
        return create_error_response(
            str(e),
...
async def _execute_clear_context_enhanced(
    ...
    try:
        ...
        if ctx:
            await ctx.info(
                f"Found {subjects_found} subjects in context '{context}'",
                logger_name="BatchOperations"
            )
            ...
            if reporter:
                async with reporter.phase("Deleting subjects", subjects_found) as phase:
                    for i, subject in enumerate(subjects):
                        try:
                            # Delete subject
                            result = registry_client.delete_subject(
                                subject,
                                context,
                                permanent=False
                            )
                            ...
                            if ctx:
                                await ctx.error(
                                    f"Exception deleting subject '{subject}': {str(e)}",
                                    logger_name="BatchOperations"
                                )
        else:
            if reporter:
        ...
        if delete_context_after and subjects_deleted == subjects_found:
            ...
            try:
                ...
                if isinstance(result, dict) and "error" in result:
                    if ctx:
                        await ctx.warning(
                            f"Failed to delete context: {result.get('error')}",
                            logger_name="BatchOperations"
                        )
                else:
                    ...
                    if ctx:
                        await ctx.info(
                            f"Successfully deleted context '{context}'",
                            logger_name="BatchOperations"
                        )
            except Exception as e:
                if ctx:
                    await ctx.error(
                        f"Exception deleting context: {str(e)}",
                        logger_name="BatchOperations"
                    )
...
async def count_schemas_with_progress_tool(
    ...
    try:
        if ctx:
            await ctx.debug(
                "Starting schema count operation",
