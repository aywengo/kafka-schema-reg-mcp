...
async def simulate_data_processing(
    ...
    reporter = ProgressReporter(task_id, context) if task_id else None
    processed_items = []
    ...
    try:
        ...
        if context:
            await context.info(
                f"Successfully processed {len(processed_items)} items",
                logger_name="DataProcessor"
            )
...
async def main():
    ...
    await enhanced_task_manager.execute_task(
        task,
    ...
    task2 = create_context_aware_task(
        TaskType.SYNC,
        metadata={"description": "Legacy task"},
