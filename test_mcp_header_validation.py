...
class TestMCPProtocolVersionValidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from kafka_schema_registry_unified_mcp import validate_mcp_protocol_version_middleware
    ...
    def test_missing_header_returns_400(self):
        ...
        self.assertEqual(response.headers.get("MCP-Protocol-Version"), MCP_PROTOCOL_VERSION)
...
class TestMCPComplianceIntegration(unittest.TestCase):
    ...
    def test_compliance_status_tool(self, mock_registry_manager):
        ...
        self.assertEqual(header_validation["supported_versions"], SUPPORTED_MCP_VERSIONS)
    ...
    def test_live_server_oauth_discovery_endpoints(self):
        ...
        try:
            ...
            for endpoint in oauth_endpoints:
                response = requests.get(f"{self.server_url}{endpoint}", timeout=self.timeout)
...
class TestMCPEndpointValidation(unittest.TestCase):
    ...
    def test_mcp_endpoint_requires_header(self):
        ...
        try:
            ...
            headers = {"MCP-Protocol-Version": MCP_PROTOCOL_VERSION}
            response = requests.post(self.mcp_url, json={}, headers=headers, timeout=self.timeout)
...
def run_comprehensive_header_validation_test():
    ...
    try:
        ...
        print(f"✅ Middleware Tests: {'PASSED' if result.wasSuccessful() else 'FAILED'}")
    ...
    try:
        ...
        print(f"✅ Integration Tests: {'PASSED' if result.wasSuccessful() else 'FAILED'}")
    ...
    try:
        ...
        print(f"✅ Live Server Tests: {'PASSED' if result.wasSuccessful() else 'FAILED'}")
    ...
    all_passed = (
        test_results["middleware_tests"] and
        test_results["integration_tests"] and
        test_results["live_server_tests"]
    )
...
if __name__ == "__main__":
    ...
    print("   • Use 'python -m pytest test_mcp_header_validation.py -v' for detailed pytest output")
